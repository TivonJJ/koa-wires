...
function Wires(app,config) {
    ...
    function onAction(info,router) {
        ...
        var prefix = router.opts?router.opts.prefix||'':'';
        var routePath = prefix+info.uri;
        ...
        router.use(routerName, function*(next){
...
function parseConfig(config){
    ...
    var routerList = [];
    if(!Array.isArray(routes.route))routes.route = [routes.route];
    ...
    function parseRouteAction(route,routerConfig){
        if(!Array.isArray(route.action))route.action = [route.action];
